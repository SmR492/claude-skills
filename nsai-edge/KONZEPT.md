...
- **Trust-asymmetrische Provenienz-Übernahme (generelles Gate):** beim Merge gewinnt das Incoming die Provenienz nur bei lexikografischer Präzedenz `(effektive Stufe, Origin-Trust-Rang, asserted_confidence, origin_id)`. Ein niedriger-vertrautes Incoming kann ein höher-vertrautes Edge in KEINEM belief-relevanten Feld (source_type/asserted_at/origin) kapern. Ebenso wird die Live-Konfidenz nur durch full/authoritative-Beiträge angehoben. Damit ist die Klasse „untrusted/limited beeinflusst Belief-Werte" auf allen Achsen (Tier, Konfidenz, Recency, Provenienz) geschlossen.
