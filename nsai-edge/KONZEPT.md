...
- **S1a Substrat** (✅ geliefert, PR #36, 4-fach-Audit safe-JA): Event-Store (append-only) + Fold-Projektor (Direkt+Quelle+Band+Hash-Dedup), klassen-getrennter `event_hash`, `resolveBelief` unberührt. Konstanten: `trustPriorAlpha=3`,`trustPriorBeta=7`,`trustSourceWeight=400`,`trustAutoCorroborateCap=600`.
- **S1b Recency/Dampener** (✅ geliefert, PR #37, 5-fach-Audit safe-JA): Perioden-Modell **B (Epoche=ein decayPass)** + Cron-Hybrid; `trustOf`-Epochen-Replay mit λ-Mean-Reversion (Anti-Sleeper), Zwei-Achsen-Fold (Autorität frei / Anzahl geklemmt um `tAuth`), **gehobene abklingende Decke** (Vouch hebt auto-Decke `600→800` ∝ separater abklingender Vouch-Stärke → sleeper-frei) + Massen-Bound. Konstanten: `trustLambdaBase=300`,`trustLambdaMin=30`,`trustPerPeriodClamp=150`,`trustVouchCap=800`,`trustMassMax=100000`. **[F-vouch-persist] entschieden (Stefan): gehobene, abklingende Decke** — schließt 3 Adversarial-Runden (Sleeper-Klasse) + geerbte Trivial-Anker-Schärfe. Volle Suite 343/343 grün, const-sync 0 Drift.
...
- **S4 Defeater/Contestation** (✅ geliefert 2026-06-02, Modell H, 5 Audit-Runden): separater append-only `contestation_events`-Ledger (eigene Achse — schreibt nie `trust_events`, senkt strukturell nie den Trust); `contest()`/`contestationOf()` als **Trust-Peer-gegateter Stärke-pro-Herkunft-Fold** — nur registrierte vertraute Peers zählen (`_originTrust≠untrusted`), je nur ihre STÄRKSTE Anfechtung (per-Peer-MAX); anonym/untrusted/beliebige String-Variante (Unicode/Whitespace/Pseudonym) trägt 0 → Identität an der Peer-Registry verankert statt am freien String (schließt die Inflations-/Suppressions-Klasse R1–R4 an der Wurzel; immer-distinkt-einfügen → keine Verdrängung); `resolveBelief` reichert read-time ein `contested`-Verdikt an (`_augmentContestation`) mit Beweislast-Schwelle `thr=trustContestBase(300)+trunc(trustOf·trustContestSlope(1000)/1000)`; Gewinner hält (nie `contradicted`); eternal-Floor → `proposedContested` (Endorsement-pflichtig); Eskalation +1 Tier; safe-by-default; `peerRevoke` senkt rückwirkend. **S4b** (Auto-Erkennung „zwei gleich-fremde → contested"; as-of-historische Anfechtung), **S4c** (voll-gewichtete Peer-Trust-Aggregation statt binär) deferred.
- **Offene Review-Flags** (ADR §Kopf): [F-λmin], [F-Demut], [F-Local], [F-eternal]. ([F-contest] ✅ entschieden Modell H.)
