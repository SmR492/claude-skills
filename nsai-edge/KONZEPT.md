...
3. **Personalized PageRank:** Power-Iteration `r = (1−d)·p + d·(Wᵀr + dangling·p)`, Teleport `p` uniform auf den Seeds, Kanten-Gewicht = `confidence/1000`; **Dangling-Knoten** verteilen ihre Masse über `p` (kein Rank-Leck); Abbruch bei L1-Delta < `tol` ODER `max_iter` (dann `converged=false` im Ergebnis).
