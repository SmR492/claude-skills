...
- **S2b Eltern-Attribuierung** (🔴, offen): `trust_ext`+`blame`-Propagation über `derived_from` (T.7–T.9), additiv auf S2a. **Harte Vorbedingung (S1b-Audit) bleibt:** bevor Ingest/MCP `recordAdjudication` verdrahtet, müssen `human_*`/`oracle_higher_tier` nicht-remote-injizierbar sein (Stefan-Gate).
