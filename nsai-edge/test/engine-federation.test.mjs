...
function signedWire(idEngine, originId, { s, p, o, asserted = 800, confidence = null, vc = { x: 1 }, sourceType = 'manual', assertedAt = '2025-01-01T00:00:00Z' }) {
  const t = {
    wire_version: 1, triple_hash: tripleHash(s, p, o), subject: s, predicate: p, object: o,
    asserted_confidence: asserted, temporality: 'stable', source_type: sourceType, asserted_at: assertedAt,
    origin_peer_id: originId, derived_from: null,
  };
  const sig = signTriple(idEngine.identity.privateKeyPem, t);
  return { ...t, confidence: confidence ?? asserted, relayed_by: originId, vector_clock: vc, signature: sig };
}

test('F1.9: nicht an den Key gebundener origin_peer_id (Key-Confusion) wird abgewiesen', () => {
  const victim = fresh();
  const attacker = fresh();
  // Angreifer registriert seinen Key unter einem FREI gewählten, nicht-gebundenen peer_id und trusted ihn.
  victim.peerAdd('peer:trustedbank0', attacker.identity.publicKeyPem);
  victim.peerTrust('peer:trustedbank0', 'full');
  const wire = signedWire(attacker, 'peer:trustedbank0', { s: 'Konto', p: 'gehoert', o: 'Opfer', vc: { 'peer:trustedbank0': 1 } });
  const res = victim.receiveIngest([wire]);
  assert.equal(res[0].status, 'rejected'); // Bindung peer_id==peer:fp greift
});

test('F1.9: korrekt gebundener Origin wird akzeptiert (Gegenprobe)', async () => {
  const { A, B } = pair('full');
  A.storeTriple({ subject: 'Faktum', predicate: 'ist', object: 'Wahr', confidence: 900 });
  const tally = await B.pull(link(A), A.peerId);
  assert.equal(tally.accepted, 1);
});

test('F1.11: implausibel hoher Vector-Clock-Sprung wird abgewiesen (Clock-Vergiftung)', () => {
  const c = fresh();
  c.peerAdd('peer:p', fresh().identity.publicKeyPem); c.peerTrust('peer:p', 'full');
  const wire = { wire_version: 1, triple_hash: tripleHash('Aa', 'ist', 'Bb'), subject: 'Aa', predicate: 'ist', object: 'Bb',
    asserted_confidence: 800, confidence: 800, source_type: 'manual', asserted_at: '2025-01-01T00:00:00Z',
    temporality: 'stable', origin_peer_id: 'peer:p', relayed_by: 'peer:p', vector_clock: { 'peer:p': 9999999999 }, derived_from: null, signature: 'ed25519:x' };
  assert.equal(c.mergeIncoming(wire, { peerTrust: 'full' }), 'rejected');
});

test('F1.15: unsignierte Live-Konfidenz wird auf asserted_confidence gedeckelt', () => {
  const c = fresh();
  const hash = tripleHash('Thema', 'ist', 'Xx');
  const base = { wire_version: 1, triple_hash: hash, subject: 'Thema', predicate: 'ist', object: 'Xx',
    temporality: 'stable', source_type: 'manual', asserted_at: '2025-01-01T00:00:00Z', derived_from: null, signature: 'ed25519:x' };
  // Bestehender Edge (full) mit Konfidenz 300.
  c.mergeIncoming({ ...base, asserted_confidence: 300, confidence: 300, origin_peer_id: 'peer:a', relayed_by: 'peer:a', vector_clock: { 'peer:a': 1 } }, { peerTrust: 'full' });
  // Incoming (full) behauptet asserted 400, aber Live-Feld 1000 → darf nur auf 400 anheben, nicht 1000.
  c.mergeIncoming({ ...base, asserted_confidence: 400, confidence: 1000, origin_peer_id: 'peer:b', relayed_by: 'peer:b', vector_clock: { 'peer:b': 1 } }, { peerTrust: 'full' });
  assert.equal(c._getEdge(hash).confidence, 400);
});

test('F1.13: übernimmt ein untrusted-Origin die Provenienz, wird der Record quarantänisiert', () => {
  const c = fresh();
  const hash = tripleHash('Sub', 'ist', 'Yy');
  const base = { wire_version: 1, triple_hash: hash, subject: 'Sub', predicate: 'ist', object: 'Yy',
    temporality: 'stable', source_type: 'manual', asserted_at: '2025-01-01T00:00:00Z', derived_from: null, signature: 'ed25519:x' };
  // Bestehender untrusted-Record, künstlich auf active gesetzt.
  c.mergeIncoming({ ...base, asserted_confidence: 300, confidence: 300, origin_peer_id: 'peer:u1', relayed_by: 'peer:u1', vector_clock: { 'peer:u1': 1 } }, { peerTrust: 'untrusted' });
  c.db.prepare("UPDATE knowledge_edges SET local_status='active' WHERE triple_hash=?").run(hash);
  // Untrusted-Incoming gewinnt (höhere asserted, gleicher Rang) → Provenienz untrusted → quarantäne.
  c.mergeIncoming({ ...base, asserted_confidence: 500, confidence: 500, origin_peer_id: 'peer:u2', relayed_by: 'peer:u2', vector_clock: { 'peer:u2': 2 } }, { peerTrust: 'untrusted' });
  assert.equal(c._getEdge(hash).local_status, 'quarantined');
});
