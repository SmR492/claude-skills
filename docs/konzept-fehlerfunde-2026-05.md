...
- **Kandidat F-1 — `SUGG_CAP=50` nicht gespiegelt:** §G.2 (Z.730) nennt `SUGG_CAP=50` als parität-relevant, `LEARN_CONSTANTS` (conformance.mjs:65-69) enthält es nicht. Gleiche Omissions-Klasse wie sourceTier. (Existenz im Code unbestätigt — engine.mjs mid-write.) — **✅ ERLEDIGT:** `SUGG_CAP`+`LIMIT 20` waren lokale Magic-Literale in `learnTrustAdjustments`; in den Spec gehoben (`suggestionCap=50`, `suggestionEvidenceCap=20`), Engine liest daraus, in `LEARN_CONSTANTS` gespiegelt (AC-R7.13). §G.2-/AC-20.12-Widerspruch versöhnt (lokaler Op; Spiegel nur bindend, wenn PHP ein eigenes `learnTrustAdjustments` anbietet).
- **Kandidat F-2 — Status-Drift §H:** UC-EP-ACs (AC-10.1–10.9) als „offen (Slice #2)" markiert, obwohl test/engine-episodic.test.mjs existiert + Retro 0006 Slice abgeschlossen. Wahrscheinlich „gebaut-im-Code, offen-im-Konzept". Vor Aktion verifizieren. — **✅ ERLEDIGT (voll):** deterministische Coverage-Map aus den Test-Titeln (AC-ID-benannt, inkl. kombinierter Namen `AC-X/AC-Y`) gegen alle 11 §H-Slices. Ergebnis: §H von ~138 Blanko-„offen" → **131 grün** (mit Test-Datei-Referenz) + **7 offen mit explizitem Grund** (6 echte Test-Lücken: AC-10.11, 12.5, 15.8, 15.9, 15.11, 21.16; 1 deferred: AC-15.15 #M.2-Endorsement-Revocation). „offen" in §H heißt ab jetzt IMMER „dokumentierte Lücke oder deferred", nie „vergessen". Surfacing-Wert: 6 reale Test-Lücken sichtbar gemacht.
