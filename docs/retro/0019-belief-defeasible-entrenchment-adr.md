...
### 4.3 Recency / Decay (O4, [F-λmin])
`λ_eff(trust) = max( λ_min , trunc(λ_base · (1000 − trust) / 1000) )`, `λ_min > 0`. Hoher Trust → langsamer Fade (Verankerung „das Wahre bleibt", #4); `λ_min>0` garantiert, dass **auch eine trust=1000-Quelle bei kompletter Stille in endlicher Zeit zurückdriftet** (Anti-Sleeper, löst die Kollision mit 07 §6). Mean-Reversion der Beta-Bilanz zum Prior. **Integer-Hinweis (07 §6):** verlässt reine Integer-Counts → **Fixed-Point-Milli-Counts**, deterministischer Zeit-Anker = vorhandenes Konfidenz-Decay (wiederverwendet); im Konstanten-Konsistenz-Gate führen.

### 4.4 Oracle-Gating / „Anzahl zählt nie" (Inv. 6)
Nur `human_*`/`oracle_higher_tier` erzeugen signifikante Verschiebungen. `auto_corroborate` **gebändert**: hebt ein Item asymptotisch **bis exakt 600‰ (Obergrenze ‚plausibel'), nie ins ‚solide'-Band (≥601)**; korrelierte Items über gemeinsamen Herkunfts-/Inhalts-Hash = **ein** Signal. Die `/n`-Verdünnung (§4.2) ist die formale „Anzahl-zählt-nie"-Mechanik.
...
| O5 | auto_corroborate-Kappe exakt 600‰; Hash-Dedup |
...
|---|---|---|---|---|
| **S1 Substrat** | Event-Store (lokal, append-only) + Fold-Projektor **nur Direkt+Quelle+Recency+Band+Dampener** (integer-promille, total-geordnet, safe-by-default). **KEINE Eltern-Attribuierung/`trust_ext`** (das ist S2). `resolveBelief` unberührt. | T.1–T.6,T.10,T.11,T.13 | 🟡 | nein (additiv) |
...
| (6) Anzahl zählt nie | ✅ in **S1** über auto_corroborate-Band + Hash-Dedup (§4.4); ab **S2** zusätzlich über `/n`-Verdünnung (§4.2) |
