...
**Rückschreib-Pflicht (Wissens-Akkumulation):** Was in Tier 1 (MCP) FEHLTE, aber über Tier 2 (Web) gefunden wurde, MUSS **verifiziert** (gegen Leitplanken + Realität, nie blind übernehmen) und dann **VOLLSTÄNDIG** (nicht fragmentarisch) via MCP in die Wissensbasis eingetragen werden — mit Provenienz (Quelle + Quelltyp `web`/`fachquelle`), sodass es beim nächsten Lookup Tier 1 ist. So wächst der kuratierte Graph; das Modell rät nicht zweimal dasselbe. **Schreib-Tools (nsai-edge):** kuratierte Fakten via `store_triple` (mit `source_type`-Provenienz) · Erkenntnisse/Verläufe via `record_episode` · Korroboration eines bestehenden Fakts via `endorse` (Anzahl-Achse, gedeckelt). **Two-Door-Grenze (KI schreibt vs. Mensch vollzieht):** Die KI darf FREMDE Fakten/Trust NIE autonom herabstufen/retraktieren — gefährliche Änderungen (reject · supersede/set_validity an Fremd-Edges · Autoritäts-Endorse · peer_trust · Fiktions-Promotion) laufen über `propose_*` → Mensch-`approve` (CLI). Anfechtung via `contest` (kippt nur das `contested`-Verdikt, senkt nie Trust). Eigene Fakten (self-origin) darf die KI direkt pflegen.
